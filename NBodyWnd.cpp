...
void NBodyWnd::DrawBodies()
{
    ...
    const PODState *state = reinterpret_cast<PODState*>(m_pSolver->GetState());
    const PODAuxState *state_aux  = m_pModel->GetAuxState();

    std::vector<PODState> pos;
    std::vector<PODState> neg;

    for (int i=0; i<m_pModel->GetN(); ++i)
    {
        if(state_aux[i].mass>0) pos.push_back(state[i]);
        else                    neg.push_back(state[i]);
    }
    ...
    glBegin(GL_POINTS);
    glColor3f(1,1,1);
    for( auto &xy: pos ) glVertex2f( xy.x, xy.y );
    glColor3f(1,0.1,0.1);
    for( auto &xy: neg ) glVertex2f( xy.x, xy.y );
    glEnd();
}
